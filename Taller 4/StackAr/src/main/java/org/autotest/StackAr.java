...
public class StackAr extends Stack {
	...
	public boolean repOK() {
		try {
			// COMPLETAR
			boolean elemsNoNull = elems != null;
			boolean indexEnRango = readIndex >= -1 && readIndex < elems.length;
			boolean fueraDeRangoCorrecto = true;
			for (int i = readIndex+1; i < elems.length; i++) {
				fueraDeRangoCorrecto = fueraDeRangoCorrecto && elems[i] == null;
			}
			return elemsNoNull && indexEnRango && fueraDeRangoCorrecto;
